...
INSERT INTO players (id, name, price)
VALUES (?, ?, ?);
...
INSERT INTO my_team (id, name, score)
VALUES (?, ?, ?);
...
SELECT p.*
FROM players p
  INNER JOIN my_team_players mtp ON p.id = mtp.player_id
WHERE mtp.my_team_id = ?;
-- name: FindAllPlayersByIDs :many
SELECT *
FROM players
WHERE id IN (?);
-- name: DeleteAllPlayersFromMyTeam :exec
DELETE FROM my_team_players
WHERE my_team_id = ?;
-- name: AddPlayerToMyTeam :exec
INSERT INTO my_team_players (my_team_id, player_id)
VALUES (?, ?);
